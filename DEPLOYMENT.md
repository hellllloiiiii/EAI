...
The site is now live worldwide. The whole course works immediately; progress just saves per-device until you finish steps 4–6, which turn on accounts so learners can keep their progress across phones and computers.
...
2. Open **SQL Editor → New query**, paste the entire contents of [supabase/schema.sql](supabase/schema.sql), click **Run**. It creates the learner profile table and its security rules.
...
4. **Authentication → Sign In / Up**: leave **Confirm email** ON. Under **Attack protection**, enable leaked-password protection.
...
On the live URL:
1. Without logging in: the whole course works, progress ring moves as you open lessons.
2. **Create account** with your email → confirmation email arrives → click the link → **Log in** on the site.
3. Fill in the profile, click **Personalize course**, open a few lessons across levels.
4. Hard-refresh, then open the site in a private/incognito window and log in again — profile, level, and progress should follow the account.
5. In Supabase **Table Editor**: `profiles` has your row with `viewed_modules` filled in.
